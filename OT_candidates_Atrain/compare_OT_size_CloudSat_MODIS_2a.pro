...
pro compare_OT_size_CloudSat_MODIS_2a
 ...
 ; in this version using MERRA2 tropopause height
  ...
  dir='/data/keeling/a/yulanh/c/BW_backup/CloudSat/2B-GEOPROF/2008/'
...
;	if i lt 300 then goto,jump1
	...
	cldclass_fname='/data/keeling/a/yulanh/c/BW_backup/CloudSat/2B-CLDCLASS-LIDAR/2008/'+strflag+'_CS_2B-CLDCLASS-LIDAR_GRANULE_P1_R05_E02_F00.hdf'
	read_dardar,cldclass_fname,'CloudLayerTop',cldtop ; used for modis parrax-correlction

	; get tropopause 
	trop_hgt_fname='/data/keeling/a/yulanh/c/BW_backup/mydata/OT_modis/CCM/Tropopause_top/Tropopause_cldtop_'+strflag+'_lr.sav' 
    ...
    mod03_dir = '/data/gdi/e/MODIS/MYD03/'+year+$
		'/'+day+'/'
	mod03_fname=file_search(mod03_dir,'*'+year+day+'.'+hhmm+'.*')
    mod02_dir = '/data/gdi/e/MODIS/MYD021KM/'+year+$
		'/'+day+'/'
...
;	  	 spawn,'sh modis_dn_myd06_2a.sh'
 ;   	mod03_dir = '/data/keeling/a/yulanh/c/BW_backup/mydata/OT_modis/CCM/post_process_OT/MYD03/'+year+$
;		'/'+day+'/'
;		mod03_fname=file_search(mod03_dir,'*'+year+day+'.'+hhmm+'.*')
 ;   	mod02_dir = '/data/keeling/a/yulanh/c/BW_backup/mydata/OT_modis/CCM/post_process_OT/MYD021KM/'+year+$
;		'/'+day+'/'
;		mod02_fname=file_search(mod02_dir,'*'+year+day+'.'+hhmm+'.*')
;	endif
...
;	print,mod03_fname
	...
	caldat,Julday(1,float(day),2008),month,date_day
	month_1=strmid(strcompress(string(month/100.0)),3,2)
	date_day_1=strmid(strcompress(string(date_day/100.0)),3,2)
	
	merra2_fname=strcompress('/data/keeling/a/yulanh/c/MERRA-2/2008/'+'MERRA2_300.inst1_2d_asm_Nx.2008'+$
...
;	modis_bt67_gs=gauss_smooth(bt67,1)
 ...
 ;               print,hi,ind1[hi],Ncontinues
		...
		if (si ge 0) and (si le Ns-1) then begin	
			tpcldlat=lat[si]
			tpcldlon=lon[si]
			;cth_parallax=topmost_dbz[si-tplowscp]
...
;			endif
			...
			if (topmost[si] le 0 and diffdis[disind[0]] le 0.05) then begin ; if topmost height lt 0, no parallax data
				colocate_bt11[ii] = modis_bt11_gs[res_scp[0],res_scp[1]]
				colocate_bt67[ii] = modis_bt67_gs[res_scp[0],res_scp[1]]
				colocate_ref062[ii]=rad_062[res_scp[0],res_scp[1]]
...
;			print,ii,diffdis[disind[0]],tpcldlat,tpcldlon,modlon_parallax[res_scp[0],res_scp[1]],modlat_parallax[res_scp[0],res_scp[1]],modis_bt11_gs[res_scp[0],res_scp[1]]
	...
	;first derivative 
	first_deriv_bt11=deriv(x*0.1,colocate_bt11)
	;second derivative
...
;	ct=colorbar(target=c1,position=[pos[0]+0.1,pos[3]-0.15,pos[2]-0.1,pos[3]-0.13])
	...
	for oti=0,OTnum-1 do begin
		...
		tpbt11sm=colocate_bt11[tpotscp_1]
		...
		if (tpotscp_1[btind] ge 15 and tpotscp_1[btind] le tpNs-15) then begin
			...
			OT_meanbt.add,mean(colocate_bt11[left_scp:right_scp]) ; second derivative method


		    ;tphick cirrus size and bt11, based on second derivative method
			left_bt=reverse(colocate_bt11[0:tpotscp_1[btind]])
			right_bt=colocate_bt11[tpotscp_1[btind]:tpNs-1]
			btd_67_11=colocate_bt67-colocate_bt11
			...
			cirrus_bt=(total(colocate_bt11[cileft_scp:left_scp-1])+total(colocate_bt11[right_scp+1:ciright_scp]))/$
			(n_elements(colocate_bt11[cileft_scp:left_scp-1])+n_elements(colocate_bt11[right_scp+1:ciright_scp]))
			
			if cileft_scp lt left_scp-1 and ciright_scp le right_scp+1 then $
			cirrus_bt=(total(colocate_bt11[cileft_scp:left_scp-1]))/$
			(n_elements(colocate_bt11[cileft_scp:left_scp-1]))
	
			if cileft_scp ge left_scp-1 and ciright_scp gt right_scp+1 then $
			cirrus_bt=(total(colocate_bt11[right_scp+1:ciright_scp]))/$
			(n_elements(colocate_bt11[right_scp+1:ciright_scp]))
...
;			endif	
			...
			OT_meanbt_merra_modis.add,mean(colocate_bt11[merraot_leftscp:merraot_rightscp])
			...
			OT_center_BT11.add,colocate_bt11[tpotscp_1[btind]]
			OT_center_BT67.add,colocate_bt67[tpotscp_1[btind]]
	...
	save,colocate_bt11,colocate_bt67,colocate_bt11,colocate_bt67,$
	colocate_lat_parallax,colocate_lon_parallax,tptropopause,tptropopause_t,topmost_dbz,tptopmost,filename=strcompress(strmid(png_fname,0,25)+'_parallax_colocated_information.sav',/rem)
...
stop
  ...
  save,OT_lon1,OT_lat1,OT_size_Cldsat1,OT_size_modis1,OT_center_BT11_1,OT_center_BT67_1,OT_vis_1,OT_above_trop_cld_1,OT_above_trop_mod_1,trop_merra2_T1,$
	...
	filename=strcompress('CloudSat_MODIS_OT_2008'+string(i)+'_parallax_dbz25_update.sav',/rem)
  print,systime()
  stop
