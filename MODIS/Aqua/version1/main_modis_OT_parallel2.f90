    ...
    real :: tp_ave_otbt11,tp_ave_otbt67,tp_ave_cibt11,tpot_area,&
            tpot_proba,tp_std_otbt11,tp_std_cibt11,tp_std_otci,&
            tp_tropopause_t,tpfx
    real :: tpdata(30)
...
!        print *, merra2fname
       ...
       ! OT_area=0.0
        ...
        !print *,'bt11 shape',shape(BT11),BT11(1,1)
        ...
        !read(myday(49:50),'(i2)') modhh
        !read(myday(51:52),'(i2)') modmm
       ...
       ! print *,'1',myid,modhh,modmm,hour_scp,myday
        ...
        !wfname='/data/keeling/a/yulanh/c/OT_output/Aqua/MODIS_MYDOTinfo_'//myday(41:53)//'txt'
        wfname='/data/keeling/a/yulanh/c/OT_output/Aqua/MODIS_MYDOTinfo_'//myday(48:60)//'txt'
                ...
                .and. colupres <= modNcol) Then
                    ...
                    IF (bmod_read_flag ==0) Then 
                        ...
                        IF (modmm .lt. 5) Then
                                ...
                                bmodmm=55
                                Julday_b=Julday_1-1
                                write(Julday_bs,'(i0)') Julday_b
                                if (len(trim(Julday_bs)) == 1) &
                                Julday_bs='00'//trim(Julday_bs)
                                if (len(trim(Julday_bs)) == 2) &
                                Julday_bs='0'//trim(Julday_bs)
                                !print *,Julday_as
                                endif
                        EndIf
                        ...
                        byyddhhmm=yrindex//Julday_bs//'.'//bmodhh_1//bmodmm_1
              

                       call system("ls "//"/data/gdi/f/yulanh/MODIS/MYD021KM/"//&
                                yrindex//"/"//Julday_bs//"/"&
                                //"MYD021KM.A"//byyddhhmm//"*"//" &
                                > mod02file"//byyddhhmm,status)

                        open(500,file="mod02file"//byyddhhmm)
                        read(500,fmt="(a87)",iostat=status) bmod02fname
                        close(500)
                ...
                colupres <= modNcol) Then
                        ...
                        IF (modmm .eq. 55) Then 
                            ...
                            if (amodhh == 24) then
                                amodhh=0
                                Julday_a=Julday_1+1
                                write(Julday_as,'(i0)') Julday_a
                                if (len(trim(Julday_as)) == 1) &
                                Julday_as='00'//trim(Julday_as)
                                if (len(trim(Julday_as)) == 2) &
                                Julday_as='0'//trim(Julday_as)
                            endif        
                        EndIf
        ...
        !   print *,'3 ',values
                ...
                !print *,tpbt11_box_sm(:,33:41)
                 ...
                 IF ((tp_ave_otbt67 > tp_ave_otbt11) .and. (tp_ave_cibt11 > tp_ave_otbt11)) Then
                         ...
                         tpot_area=pi*(float(sum(OT_edge))/count(OT_edge > 0))**2.0
                 ...
                 !===== get the bt for the array of bt arround 1000 km 
                        ...
                        write(1000,*) colj,rowi,tplon,tplat,tpot_proba,tpot_area,&
