        subroutine read_modis02_b(fname)

        use global, only: modNrow_b,modNcol_b,rad_1000emis_b
	...
	!	status=sfendacc(sd)
		...
		modNrow_b = dim_sizes(2)
		modNcol_b = dim_sizes(1)
                
		allocate(rad_1000emis_1(dim_sizes(1),dim_sizes(2),dim_sizes(3)))
		allocate(rad_1000emis_b(dim_sizes(1),dim_sizes(2),dim_sizes(3)))
		...
		!print *,offset	
		Do ranki=1,dim_sizes(3)
		rad_1000emis_b(:,:,ranki)=(rad_1000emis_1(:,:,ranki)-offset(ranki))*vscale(ranki)
		Enddo 
