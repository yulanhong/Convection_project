        subroutine read_modis02_a(fname)

        use global, only: modNrow_a,modNcol_a,rad_1000emis_a
	...
	!	status=sfendacc(sd)
		...
		!print *,NROW,dim_sizes
                ...
                edges(3)=dim_sizes(3)
                stride(1:3)=1
		modNrow_a = dim_sizes(2)
		modNcol_a = dim_sizes(1)

		allocate(rad_1000emis_1(dim_sizes(1),dim_sizes(2),dim_sizes(3)))
		allocate(rad_1000emis_a(dim_sizes(1),dim_sizes(2),dim_sizes(3)))
		...
		!print *,offset	
		Do ranki=1,dim_sizes(3)
		rad_1000emis_a(:,:,ranki)=(rad_1000emis_1(:,:,ranki)-offset(ranki))*vscale(ranki)
		Enddo 
